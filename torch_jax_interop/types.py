...
class Module(Protocol[P, Out_cov]):
    """Protocol for a torch.nn.Module that gives better type hints for the
    `__call__` method."""
...
class Dataclass(metaclass=_DataclassMeta):
    ...
    This plays nicely with @functools.singledispatch, allowing us to
    register functions to be used for dataclass inputs.
    """
...
def is_sequence_of(
    ...
    """Used to check (and tell the type checker) that `object` is a sequence of
    items of type `V`."""
    try:
...
def is_list_of(
    ...
    """Used to check (and tell the type checker) that `object` is a list of
    items of this type."""
    return isinstance(object, list) and is_sequence_of(object, item_type)


def jit(fn: Callable[P, Out]) -> Callable[P, Out]:
    """Small type hint fix for jax's `jit` (preserves the signature of the
    callable)."""
...
def value_and_grad(  # type: ignore
    ...
    """Small type hint fix for jax's `value_and_grad` (preserves the signature
    of the callable)."""
